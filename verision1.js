...
// "press spacebar to stir". Därefter bestämmer den bildens koordinater.
...
function startGame() {
  if (loaded && !imageShowing) {
    ...
    bowlY = canvas.height / 1.8 - img.height / 2; // /1.8 för att skålen ska hamna lite lägre ner
    // än precis i mitten.
...
// enskild flinga rör sig längs denna bana.
function moveCerealandSpoon(t) {
  if (willStir && gameStarted) {
    for (let i = 0; i < cereal.length; i++) {
      ...
      console.log(cereal[i].cy + 15, spoon[0].ep1y);
    }
    drawMovingSpoon(t);
...
// Funktionen ritar ut skeden som ska snurra runt. Detta sker
// genom att skedens koordinater följer en ellipsebana
// och skeden rör sig runt denna bana.
